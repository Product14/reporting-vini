/* Inbox — chatbot/receptionist handover: claim / hand back a chat-service session.
 *   POST /chat/sessions/:sessionId/handover/toggle   (Bearer) → { sessionId, phase, claimedByUserId, claimedByName }
 * The endpoint infers the direction from the session's current phase (PENDING/NONE → claim, ACTIVE → hand
 * back). 409 = someone else claimed it; 404 = unknown session — both forwarded verbatim so the client can react.
 * Proxied as:
 *   POST /api/inbox/chat/toggle?team_id=&enterprise_id=&env=&session_id=   body: { sessionId }
...
export async function POST(request: Request): Promise<Response> {
  ...
  const res = await spyneServiceSend<unknown>(
    `/chat/sessions/${encodeURIComponent(sessionId)}/handover/toggle`,
