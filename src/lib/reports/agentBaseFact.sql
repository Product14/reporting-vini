...
--   • {RESELLER_SCOPE} — the "real rooftops only" reseller screen, substituted at load time from
--                src/lib/reports/enterpriseScope.ts (the single source of truth, shared with
--                detailQueries.ts). Reseller-owned enterprises are excluded EXCEPT those explicitly
--                allowlisted there as real paying customers. Like {START}/{END}, this file is not
--                paste-runnable as-is — load it through spineSql.ts.
--   • {TEAM_SCOPE} — `1=1` for a normal fleet-wide run; a `l.team_id IN (…)` predicate when the ETL is
--                invoked for specific rooftops (backfill.ts --teams/--enterprise). Purely an OPTIMIZER
--                hint: nearly every downstream CTE joins lead_canonical, so narrowing it there prunes
--                the scan instead of filtering a fleet-wide result at the end. Scoped correctness does
--                NOT depend on it — backfill.ts also wraps the spine in a team filter and asserts no
--                out-of-scope team survives, so a lost substitution costs time, never isolation.
