...
// Small "Download ▾" dropdown offering Print/PDF, CSV and XLSX — styled to match DateFilter's popover in
// kit.tsx. Kept generic (just callbacks) so any report tab can drop it in next to its own data.
export function ExportMenu({ onPrint, onCSV, onXLSX }: { onPrint: () => void; onCSV: () => void; onXLSX: () => void }) {
  ...
  return (
    <div ref={ref} className="relative shrink-0">
      ...
      {open && (
        <div className="absolute right-0 top-full z-30 mt-2 flex min-w-[180px] flex-col gap-0.5 rounded-xl border border-[#e5e7eb] bg-white p-1.5 shadow-[0_10px_30px_rgba(16,24,40,0.15)]">
          <button
