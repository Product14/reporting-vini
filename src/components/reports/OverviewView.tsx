      ...
      node: (
        <div className="flex flex-col gap-3.5">
          ...
          <div className="grid gap-3" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(190px, 1fr))" }}>
            ...
            <Hideable id="tile.appts" ctrl={ctrl}><ValueTile label="Appointments — AI-booked" total={fmtInt(fleet.appointments)} inbound={fmtInt(split.inbound.appointments)} outbound={fmtInt(split.outbound.appointments)} unassigned={fleet.appointments - split.inbound.appointments - split.outbound.appointments > 0 ? fmtInt(fleet.appointments - split.inbound.appointments - split.outbound.appointments) : undefined} delta={fleet.deltas.appointments} accent="green" subtext={fleet.appointmentsAssisted > 0 ? <>+{fmtInt(fleet.appointmentsAssisted)} AI-assisted (CRM)</> : <>meeting created by the AI</>} onClick={fleet.appointments > 0 ? openApptModal : undefined} /></Hideable>
          </div>
