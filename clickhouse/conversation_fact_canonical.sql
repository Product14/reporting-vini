-- GENERATED by scripts/gen-conversation-fact-mv.ts — DO NOT HAND-EDIT.
...
-- History floor: 2025-09-01   Refresh: EVERY 30 MINUTE   Columns: 48

CREATE DATABASE IF NOT EXISTS vini_reporting;
...
CREATE TABLE IF NOT EXISTS vini_reporting.conversation_fact_canonical (
  `conversationId` Nullable(String),
  `callId` Nullable(String),
  `conv_type` Nullable(String),
  ...
  `agent_type` String,
  `activity_day` Date,
  `activity_ts` Nullable(DateTime),
  `lead_created_at` DateTime64(3),
  `lead_source` Nullable(String),
  `touched` UInt8,
  `is_call` UInt8,
  `is_sms` UInt8,
  `is_chat` UInt8,
  `n_sms_messages` UInt64,
  `n_human_inbound` UInt64,
  `n_sms_outbound` UInt64,
  `qualified_via_call` UInt8,
  `qualified_via_sms` UInt8,
  `qualified_via_chat` UInt8,
  `qualified` UInt8,
  `appointment_booked` UInt8,
  `appointments_count` UInt64,
  `appointment_assisted` UInt8,
  `appointments_assisted_count` UInt64,
  `connected` UInt8,
  `sms_replied` UInt8,
  `reached_person` UInt8,
  `primary_intent` String,
  `query_resolved` UInt8,
  `had_appt_intent` UInt8,
  `had_transfer` UInt8,
  `transferred` UInt8,
  `transfer_failed` UInt8,
  `had_callback` UInt8,
  `talk_seconds` Int64,
  `quality_score` Nullable(Float64),
  `opted_out_sms` UInt8,
  `opted_out_call` UInt8,
  `after_hours` Nullable(UInt8),
  `is_speed_to_lead` UInt8,
  `speed_to_lead_response_time` Nullable(Int64)
)
ENGINE = SharedMergeTree
PARTITION BY toYYYYMM(activity_day)
ORDER BY (team_id, agent_type, activity_day);

-- The refreshable MV. Full recompute every 30 MINUTE; the window is [2025-09-01, today()+1) so it always
...
CREATE OR REPLACE MATERIALIZED VIEW vini_reporting.rmv_conversation_fact_canonical
REFRESH EVERY 30 MINUTE
TO vini_reporting.conversation_fact_canonical
AS
...
--
--   • toDate('2025-09-01') / today() + 1  — date-window placeholders bounding toDate(createdAt) to [START, END),
...
--   • (ed.reseller_id IS NULL OR ed.reseller_id = '' OR ed.enterprise_id IN ('62f962c8e')) — the "real rooftops only" reseller screen, substituted at load time from
...
--                allowlisted there as real paying customers. Like toDate('2025-09-01')/today() + 1, this file is not
--                paste-runnable as-is — load it through spineSql.ts.
--   • 1=1 — `1=1` for a normal fleet-wide run; a `l.team_id IN (…)` predicate when the ETL is
...
callback_from_outbound AS (
    ...
    -- dealer_leads.endcallreports (joined by callId); floored by toDate('2025-09-01') to match the spine window
    ...
    WHERE ecr.__deleted = 0
      ...
      AND toDate(ecr.createdAt) >= toDate('2025-09-01') AND toDate(ecr.createdAt) < today() + 1
...
lead_canonical AS (
    ...
    WHERE l.is_deleted = 0 AND l.__deleted = 0
      ...
      AND (ed.reseller_id IS NULL OR ed.reseller_id = '' OR ed.enterprise_id IN ('62f962c8e'))
      AND 1=1
...
lead_high_intent_action AS (
    ...
    WHERE ai.__deleted = 0
      ...
      AND ai.createdAt >= toDate('2025-09-01') AND ai.createdAt < today() + 1
),
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE is_active = 1 AND __deleted = 0 AND source = 'spyne'
              ...
              AND toDate(created_at) >= toDate('2025-09-01') - 1 AND toDate(created_at) < today() + 1 + 1
        ) AS m
        JOIN (
            ...
            WHERE __deleted = 0 AND ifNull(isTest, 0) = 0 AND lower(type) = 'chat'
              ...
              AND toDate(createdAt) >= toDate('2025-09-01') AND toDate(createdAt) < today() + 1
        ) AS c
...
conv_src AS (
    ...
    WHERE ifNull(c.isTest, 0) = 0 AND c.__deleted = 0
      ...
      AND toDate(c.createdAt) >= toDate('2025-09-01') AND toDate(c.createdAt) < today() + 1
),
...
appt_attribution AS (
    ...
    FROM (
        ...
        WHERE m.is_active = 1 AND m.__deleted = 0
          ...
          AND toDate(m.created_at) >= toDate('2025-09-01') AND toDate(m.created_at) < today() + 1
    )
...
SELECT
    ...
    -- Callback-from-outbound: flip inbound callbacks to Outbound (see callbackAttribution.ts).
...
FROM conversation_spine cs
LEFT JOIN callback_from_outbound cbo
